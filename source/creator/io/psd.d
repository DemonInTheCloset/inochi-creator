...
private
{
    BlendMode blendModeKeyToBlendingMode(BlendingMode blendModeKey)
    {
        switch (blendModeKey)
        {
    ...
    Puppet _incImportPSD(PSD doc)
    {
        import psd : PSD, Layer, LayerType, LayerFlags, BlendingMode;
        ...
        {
            return layerGroupStack.length > 0
                ? (layerGroupStack[$ - 1].flags & LayerFlags.Visible) != 0 : false;
        }
        ...
        foreach_reverse (i, Layer layer; doc.layers)
        {
            ...
            vec2i layerPosition = vec2i(layer.left, layer.top);

            part.localTransform.translation = vec3((layerPosition.x + (layerSize[0] / 2)) - docCenter.x,
                    (layerPosition.y + (layerSize[1] / 2)) - docCenter.y, 0);

            part.enabled = (layer.flags & LayerFlags.Visible) == 0;
            part.opacity = (cast(float) layer.opacity) / 255;
            part.zSort = -(cast(float) i);
            part.blendingMode = blendModeKeyToBlendingMode(layer.blendModeKey);
            debug writeln(part.name, ": ", part.blendingMode);

            // Handle layer stack stuff
            if (layerGroupStack.length > 0)
            {
                if (isLastStackItemHidden())
                    part.enabled = false;
                if (layerGroupStack[$ - 1].blendModeKey != BlendingMode.PassThrough)
                    part.blendingMode = blendModeKeyToBlendingMode(layerGroupStack[$ - 1].blendModeKey);
            }
...
void incImportPSD(string file)
{
    ...
    try
    {
        import psd : PSD, parseDocument;

        PSD doc = parseDocument(file);

        Puppet puppet = _incImportPSD(doc);
